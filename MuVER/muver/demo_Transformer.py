"""
Transformer  # Code by wmathor, modify by WangYH-BUPT
...
# P: Symbol that will Padding in blank sequence if current batch data size is short than time steps
sentences = [
    # enc_input: 德语           dec_input: 英语       dec_output
    ['ich mochte ein bier P', 'S i want a beer .', 'i want a beer . E'],
    ['ich mochte ein cola P', 'S i want a coke .', 'i want a coke . E']
]
src_vocab = {'P': 0, 'ich': 1, 'mochte': 2, 'ein': 3, 'bier': 4, 'cola': 5}  # 5
tgt_vocab = {'P': 0, 'i': 1, 'want': 2, 'a': 3, 'beer': 4, 'coke': 5, 'S': 6, 'E': 7, '.': 8}  # 8
src_vocab_size, tgt_vocab_size = len(src_vocab), len(tgt_vocab)
word2idx = {word: idx for idx, word in enumerate(tgt_vocab)}
idx2word = {idx: word for idx, word in enumerate(tgt_vocab)}
src_max_len = max([len(enc_input_vocab.split()) for enc_input_vocab, dec_input_vocab, dec_output_vocab in sentences])
tgt_max_len = max([len(dec_input_vocab.split()) for enc_input_vocab, dec_input_vocab, dec_output_vocab in sentences])
# src_len: enc_input max sequence length,  tgt_len: dec_input(= dec_output) max sequence length
...
# Mask:
# Include: "Padding Mask"     of "Multi-Head Attention"        in Encoder and Decoder
#          "Subsequence Mask" of "Masked Multi-Head Attention" in Decoder
# 1. Padding Mask
...
def get_attn_subsequence_mask(seq):
    attn_shape = [seq.size(0), seq.size(1), seq.size(1)]  # seq: [batch_size, tgt_len]
