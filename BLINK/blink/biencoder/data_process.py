...
def get_context_representation(
...
) -> (dict, int):  # 函数应输出dict(tokens,ids),int
    mention_tokens = []
    ...
    left_quota = (max_seq_length - len(mention_tokens)) // 2 - 1  # 应当截取的上下文长度
    right_quota = max_seq_length - len(mention_tokens) - left_quota - 2
    left_add = len(context_left)  # 实际上下文长度
    ...
    mark_position = context_tokens.index(ent_start_token)    # !!记录ent_start_token在整句话的位置
...
def process_mention_data(
    ...
    for idx, sample in enumerate(iter_):
        context_tokens, m_position = get_context_representation(  # 包括tokens,ids,mark_p
            sample,
            tokenizer,
            max_context_length,
            mention_key,
    ...
    m_position = torch.tensor(
        select_field(processed_samples, "m_position"), dtype=torch.long,
    )
    data = {
    ...
    if use_world:
        data["src"] = src_vecs
        tensor_data = TensorDataset(context_vecs, cand_vecs, m_position, src_vecs, label_idx)
    else:
        tensor_data = TensorDataset(context_vecs, cand_vecs, label_idx)
    return data, tensor_data
    # tensor_data: {
    #   context: [101,.,1,.,2,.,102]->(sample, 128)
    #   label: [101,.,3,.102]->(sample, 128)
    #   m_position: mention开始符位置->(sample,)
    #   src: 域编号->(sample,)
    #   label_idx: src内label_id->(sample,)
    #   }
