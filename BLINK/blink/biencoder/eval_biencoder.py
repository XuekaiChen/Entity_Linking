...
def main(params):
    ...
    logger.info("Read %d %s samples." % (len(test_samples), params['mode']))
    ...
    print("finish process mention data")
    ...
    # logger.info("finish extract hard negatives")
    # logger.info("all_sample_top64_file length: %d" % new_data['labels'].size(0))
    if save_results: 
        save_data_dir = params['output_path']
        # save_data_dir = os.path.join(
        #     params['output_path'],
        #     "all_sample_top%d_candidates" % params['top_k'],
        # )
        if not os.path.exists(save_data_dir):
            os.makedirs(save_data_dir)
        save_data_path = os.path.join(save_data_dir, "%s.t7" % params['mode'])
        # 原先路径：只存了有检索的；现在：存了所有，无检索的['labels']为-1
