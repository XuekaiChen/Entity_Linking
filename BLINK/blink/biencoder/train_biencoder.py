...
def evaluate(
    ...
    for step, batch in enumerate(iter_):
        ...
        context_input, candidate_input, m_position, _, _ = batch
        with torch.no_grad():
            eval_loss, logits = reranker(context_input, candidate_input, m_position)
...
def main(params):
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            # batch = tuple(t.cuda() for t in batch)
            context_input, candidate_input, m_position, _, _ = batch
            loss, _ = reranker(context_input, candidate_input, m_position)
        ...
        utils.write_to_file(
            output_eval_file, "epoch_{}, evaluate result: {}".format(epoch_idx,results)
        )
    ...
    if params["evaluate"]:
        ...
        evaluate(params, device=device, logger=logger)
