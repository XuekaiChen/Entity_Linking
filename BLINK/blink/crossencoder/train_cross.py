...
# 按world计算准确率
def evaluate(reranker, eval_dataloader, device, logger, context_length, zeshel=False, silent=False):
    reranker.model.eval()
    if silent:
...
def main(params):
    ...
    train_data = torch.load(fname)  # {context_vecs:,candidate_vecs:,labels:,worlds:}
    context_input = train_data["context_vecs"]  # [43678, 128]
    candidate_input = train_data["candidate_vecs"]  # [43678, 64, 128]
    label_input = train_data["labels"]  # [43678]
    if params["debug"]:
        max_n = 100
    ...
    if params["zeshel"]:
        src_input = train_data['worlds'][:len(context_input)]  # world index
    ...
    context_input = valid_data["context_vecs"]  # [8380, 128]
    candidate_input = valid_data["candidate_vecs"]  # [8380, 64, 128]
    label_input = valid_data["labels"]  # [8380]
    if params["debug"]:
        max_n = 100
        context_input = context_input[:max_n]
        candidate_input = candidate_input[:max_n]
