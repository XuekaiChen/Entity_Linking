...
class CrossEncoderRanker(torch.nn.Module):
    ...
    def score_candidate(self, text_vecs, context_len):
        ...
        num_cand = text_vecs.size(1)  # [batch_size, 64, cross_context(128)]
        text_vecs = text_vecs.view(-1, text_vecs.size(-1))  # [batch_size*64, cross_context(128)]
        token_idx_ctxt, segment_idx_ctxt, mask_ctxt = to_bert_input(
            text_vecs, self.NULL_IDX, context_len,
        )
        embedding_ctxt = self.model(token_idx_ctxt, segment_idx_ctxt, mask_ctxt,)  # [batch_size*64,]

        return embedding_ctxt.view(-1, num_cand)  # [batch_size, 64]
